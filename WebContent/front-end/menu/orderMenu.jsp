  ...
  <body>
	...
	<c:if test = "${not empty errorMsgs} }">
  	...
  	<div class="container-fliud">
  		<div class="header"><h2>訂購套餐</h2></div>
  		<div class="row">
  			<div class="col-3"></div>
	  		<div class="col-6">
	  			<div class="container-fliud">
	  			...
	  			<div class="container-fliud">
			  				...
			  				<c:forEach var="chefVO" items="${listChefByMenuID}">
				  				<c:if test="${chefVO.chef_area == '0'}">
				  					<form method="post" action="<%=request.getContextPath()%>/chefSch/chefSch.do">
				  						<input type="submit" class="chef_ID btn btn-outline-secondary btn-block" name="chef_ID" value="${chefVO.chef_ID}">
			  		...
			  		<c:if test="${order_chef_sch_date!=null}">
			  			<div class="chef_sch_time">
			  				<div class=chef_sch_time_title><p>請選擇日期</p></div>
			  				<form method="post" action="<%=request.getContextPath()%>/menuOrder/menuOrder.do">
			  					...
			  					<input type="hidden" name="cust_ID" value="C00002">
			  					<input type="hidden" name="menu_ID" value="${menuVO.menu_ID}">
			  					<input type="hidden" name="action" value="insert"><br>
			  					<input type="submit" class="btn btn-outline-secondary btn-block" value="送出訂單">
			  				</form>
			  			</div>
			  		</c:if>
			  	</div>
