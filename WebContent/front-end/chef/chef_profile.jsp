...
<html lang="en">
  ...
  <body>
  	<div class="container-fliud">
  		...
  		<div class="row">
  			<div class="col-2">
  			<a href="<%=request.getContextPath()%>/menuCart/menu.jsp" class="btn btn-secondary btn-lg btn-block" tabindex="-1" role="button" aria-disabled="true">訂購套餐</a>
  			</div>
	  		<div class="col-3">
		  		<div class="card mb-3">
	  		...
	  		<div class="col-4">
		  		<div id="chef_name"><h3>${custVO.cust_name}</h3></div>	  		
	  		...
	  		<div class="col-3">
	  			<form method="post" action="<%=request.getContextPath()%>/cust/cust.do">
	  				...
	  				<input type="submit" class="btn btn-secondary btn-lg btn-block" value="編輯個人資料">
	  			</form>
	  			<form method="post" action="<%=request.getContextPath()%>/chef/chef.do">
	  				...
	  				<input type="submit" class="btn btn-secondary btn-lg btn-block" value="編輯主廚簡介">
	  			</form>	
	  			<form method="post" action="<%=request.getContextPath()%>/menuOrder/unCheckMenuOrder.jsp">
	  				<input type="hidden" name="chef_ID" value="${chefVO.chef_ID}">
	  				<input type="submit" class="btn btn-secondary btn-lg btn-block" value="查看未審核訂單">
	  			</form>
	  			<form method="post" action="<%=request.getContextPath()%>/menuOrder/unFinishedMenuOrder.jsp">
	  				<input type="hidden" name="chef_ID" value="${chefVO.chef_ID}">
	  				<input type="hidden" name="action" value="unFinishedMenuOrder">
	  				<input type="submit" class="btn btn-secondary btn-lg btn-block" value="查看未完成訂單">
	  			</form>
	  			<form method="post" action="<%=request.getContextPath()%>/chefDish/addChefDish.jsp">
	  				<input type="hidden" name="chef_ID" value="${chefVO.chef_ID}">
	  				<input type="hidden" name="action" value="updateChefDish">
	  				<input type="submit" class="btn btn-secondary btn-lg btn-block" value="管理擅長菜色">
	  			</form>
	  			<form method="post" action="<%=request.getContextPath()%>/chefSch/addChefSch.jsp">
	  				<input type="hidden" name="chef_ID" value="${chefVO.chef_ID}">
	  				<input type="hidden" name="action" value="updateChefDish">
	  				<input type="submit" class="btn btn-secondary btn-lg btn-block" value="新增主廚排程">
	  			</form>
	  		</div>
  		</div>
