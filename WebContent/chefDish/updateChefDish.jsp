  		...
  		<div class="row">
  			<div class="col-12">
				<form method = "post" action = "chefDish.do">						
					<jsp:useBean id="chefDish" scope="page" class="com.chefDish.model.ChefDishService" />
						
						<div class="form-group">
						    <label>主廚編號</label>
						    <input type="text" readonly class="form-control" name="chef_ID" value="${chefDishVO.chef_ID}" />
						</div>
						
						<div class="form-group">
						    <label>菜色編號</label>
						    <input type="text" readonly class="form-control" name="dish_ID" value="${chefDishVO.dish_ID}" />
						</div>
						...
						<div class="form-group">			
					    	<label>審核狀態</label>
					    	<select size="1" name="chef_dish_status" class="form-control">
					     		<c:forEach var="mydata" items="${mystatus}">
									<option value="${mydata}" ${(chefDishVO.chef_dish_status==mydata)? 'selected':'' } >${mydata}
								</c:forEach>
					    	</select>
					  	</div>
