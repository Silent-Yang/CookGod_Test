  		...
  		<div class="row">
  			<div class="col-12">
				<form method = "post" action = "chefSch.do">						
					<jsp:useBean id="chefSch" scope="page" class="com.chefSch.model.ChefSchService" />
						
						<div class="form-group">
						    <label>主廚編號</label>
						    <input type="text" readonly class="form-control" name="chef_ID" value="${chefSchVO.chef_ID}" />
						</div>
						
						<div class="form-group">
				    		<label>排程日期</label>
				    		<input name="chef_sch_date" readonly class="form-control" id="sch_date" type="text" value="${chefSchVO.chef_sch_date}"/>
					  	</div>
						...
						<div class="form-group">			
					    	<label>訂單狀態</label>
					    	<select size="1" name="chef_sch_status" class="form-control">
					     		<c:forEach var="mydata" items="${mystatus}">
									<option value="${mydata}" ${(chefSchVO.chef_sch_status==mydata)? 'selected':'' } >${mydata}
								</c:forEach>
					    	</select>
					  	</div>
